...
import { colorize, QueryGraph } from '@lerna-lite/core';
...
function formatColumns(resultList: ReturnType<typeof filterResultList>, viewOptions: ReturnType<typeof parseViewOptions>) {
  const formattedResults = resultList.map((result) => {
    ...
    if (result.version) {
      formatted.version = colorize('green', `v${result.version}`);
    } else {
    ...
    formatted.location = colorize('gray', relative('.', result.location));
