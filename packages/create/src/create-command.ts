import { Command, CommandType, Conf, CreateCommandOption, execSync, npmConf, ValidationError } from '@lerna-lite/core';
...
import { dirname, join as pathJoin, relative, resolve as pathResolve, posix } from 'node:path';
import { fileURLToPath } from 'node:url';
import slash from 'slash';
import parser from 'yargs-parser';

import { builtinNpmrc } from './lib/builtin-npmrc.js';
import { catFile } from './lib/cat-file.js';

const __filename = fileURLToPath(import.meta.url);
const __dirname = dirname(__filename);
const LERNA_MODULE_DATA = pathResolve(__dirname, './lib/lerna-module-data.js');
...
export function factory(argv: CreateCommandOption) {
  return new CreateCommand(argv);
}

export class CreateCommand extends Command<CreateCommandOption> {
  /** command name */
  name = 'create' as CommandType;
  ...
  constructor(argv: CreateCommandOption) {
    super(argv);
  }
  ...
  initialize() {
    ...
    this.pkgName = name;
    this.pkgsDir =
      this.project.packageParentDirs.find((pd) => pd.indexOf(pkgLocation as string) > -1) || this.project.packageParentDirs[0];

    this.camelName = parser.camelCase(this.dirName);

    // when transpiling, src => dist; otherwise everything in lib
    this.outDir = esModule ? 'dist' : 'lib';
    this.targetDir = pathResolve(this.pkgsDir, this.dirName) as string;

    this.binDir = pathJoin(this.targetDir, 'bin') as string;
    this.binFileName = (bin === true ? this.dirName : bin) as string;

    this.libDir = pathJoin(this.targetDir, esModule ? 'src' : 'lib');
    this.libFileName = `${this.dirName}.js`;

    this.testDir = pathJoin(this.targetDir, '__tests__');
  ...
  async execute() {
    ...
    if (this.options.esModule) {
      this.logger.notice(
        '✔',
        dedent`
              Ensure '${relative('.', this.pkgsDir)}/*/${this.outDir}' has been added to ./.gitignore
              Ensure rollup or babel build scripts are in the root
            `
      );
    }

    this.logger.success('create', `New package ${data.name} created at ./${relative('.', this.targetDir)}`);
  }
  ...
  resolveRelative(depNode) {
    ...
    const relPath = relative(this.targetDir, depNode.location);
  ...
  setDependencies() {
    ...
    const decideVersion = (spec) => {
      if (this.packageGraph.has(spec.name)) {
        // sibling dependency
        const depNode: any = this.packageGraph.get(spec.name);
  ...
  setFiles() {
    ...
    const files: string[] = [this.outDir];
  ...
  setHomepage() {
    ...
    const relativeTarget: string = relative(this.project.rootPath, this.targetDir);

    if (hurl.hostname.match('github')) {
      hurl.pathname = posix.join(hurl.pathname, 'tree/main', relativeTarget);
      ...
      hurl.pathname = posix.join(hurl.pathname, relativeTarget);
    }
