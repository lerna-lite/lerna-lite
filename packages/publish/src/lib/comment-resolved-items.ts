...
export async function commentResolvedItems({
  ...
  if (templates.issue) {
    const issues = await remoteSearchBy(client, 'issue', repo.owner, repo.name, lastTagOldestCommit?.commitDate || '', logger);
    issues.forEach((item) => closedLinkedIssues.add({ type: 'issue', number: item.number }));
  }

  if (templates.pullRequest) {
    const pullRequests = (
      await remoteSearchBy(client, 'pr', repo.owner, repo.name, lastTagOldestCommit?.commitDate || '', logger)
